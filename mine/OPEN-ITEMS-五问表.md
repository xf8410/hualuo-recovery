...
2. 「让红色开口说话」这套改造**生效了**：错误卷被单列在作业结尾（`grep -nE "^e: |^Caused by:|FAILURE:…"`），
   三态变量 `UNIT/BUILD/VERIFY_OUTCOME` 也正常报 `failure / skipped / skipped`；
   但同时暴露一件事——**单测一红，APK 与装机验收整段被 skip**，所以这条分支从来没产出过可装包。
3. 顺带抓到**屎山税的实锤**：作业清理阶段每个 step 都在递归 `git submodule foreach`
   （`Entering 'thirdparty/llama.cpp'` / `Entering 'thirdparty/proot'`，一次 run 里重复 6+ 遍）。
...
4. 记忆 20 项里剩下的（备份密钥静默置空、沙盒 D1–D6、生成参数上下文按条数裁剪…）逐条补五问；
